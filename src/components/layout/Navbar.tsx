...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 px-3 sm:px-8 flex items-center justify-between sticky top-0 z-20 transition-colors duration-200">
      {/* Left: Mobile Toggle & Search Bar */}
      <div className="flex items-center gap-2 max-w-[55%] sm:max-w-none">
        <button
          onClick={toggleMobileMenu}
          className="p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 md:hidden shrink-0"
          title="Buka Menu Navigasi"
        >
          <Menu className="w-6 h-6" />
        </button>

        <div className="relative w-32 xs:w-44 sm:w-80 md:w-96 transition-all">
          ...
          <input
            ...
            placeholder="Search..."
            className="w-full pl-9 pr-3 sm:pl-10 sm:pr-4 py-2 rounded-xl bg-slate-100/80 dark:bg-slate-800/80 border border-slate-200/50 dark:border-slate-700/50 text-xs sm:text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 text-slate-800 dark:text-slate-100 transition-all"
          />
          {searchQuery && (
            <button 
      ...
      <div className="flex items-center gap-2 sm:gap-4 shrink-0">
        ...
        <button 
          ...
          className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 rounded-full bg-red-500 text-white text-[11px] sm:text-xs font-bold shadow-md shadow-red-500/20 hover:bg-red-600 transition-colors animate-pulse-live cursor-pointer"
        >
        ...
        <button
          ...
          className="p-2 sm:p-2.5 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200 transition-colors relative"
        >
          {theme === 'dark' ? (
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 sm:right-0 mt-3 w-72 xs:w-80 sm:w-96 max-w-[calc(100vw-1.5rem)] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-4 z-50 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 sm:gap-3 p-1 sm:p-1.5 sm:pr-3 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
          >
            <div className="relative">
              <img
                src={currentUser.avatarUrl}
                alt={currentUser.fullName}
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-indigo-500/30"
              />
          ...
          {showRoleDropdown && (
            <div className="absolute right-0 mt-3 w-64 sm:w-72 max-w-[calc(100vw-1.5rem)] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-3.5 z-50 animate-in fade-in zoom-in-95 duration-150 space-y-3">
              {/* User Details Header */}
